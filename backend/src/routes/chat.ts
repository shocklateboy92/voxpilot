...
export const chatRouter = new Hono<AuthEnv>()
    ...
    return streamSSE(
      ...
      async (stream: SSEStreamingApi) => {
        ...
        try {
          ...
          const artifactSummaries = await getSessionArtifactSummaries(
            db,
            sessionId,
          );
          ...
          if (copilotConn) {
            for (const [toolCallId, buffered] of copilotConn.outputBuffer) {
              if (buffered) {
                const sessionName =
                  copilotConn.outputSessionNames.get(toolCallId) ?? "";
                await stream.writeSSE({
                  event: "copilot-delta",
                  data: JSON.stringify({
  ...
  .post(
    ...
    zValidator("json", SendMessageRequest),
    async (c) => {
      const sessionId = c.req.param("id");
      const db = getDb();
  ...
  .post(
    ...
    zValidator("json", ToolConfirmRequest),
    async (c) => {
      const sessionId = c.req.param("id");
      const db = getDb();
