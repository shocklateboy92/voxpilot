...
- **Config**: Environment variables with `VOXPILOT_` prefix loaded via `dotenv` in Justfile. Key settings: `llmBaseUrl` (default `http://localhost:11434/v1`), `llmApiKey` (default `ollama`), `llmDefaultModel` (default `qwen3-coder:30b`), `dbPath` (default `voxpilot.db`), `workDir` (defaults to cwd — root for all tool file access), `maxAgentIterations` (default 25).
...
| **No auth (self-hosted)** | Single-user local deployment; no session/token management needed |
...
| **OpenAI-compatible SDK** | Works with Ollama and any OpenAI-compatible inference server; configurable via `VOXPILOT_LLM_BASE_URL` |
