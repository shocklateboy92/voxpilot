...
- **DB lifecycle**: `initDb()`/`closeDb()` managed via the `db.ts` module. Single `bun:sqlite` `Database` wrapped with Drizzle ORM. `getDb()` provides the Drizzle instance to routes and services.
