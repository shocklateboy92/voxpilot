...
**Agentic coding assistant** powered by a self-hosted LLM (e.g. Qwen 3 Coder via Ollama) running on local hardware. No authentication required — the app connects directly to the configured inference server. The LLM can call read-only tools (read files, search code, list directories) through an agentic loop that executes tools and feeds results back until the task is done. Conversations (including tool call/result messages) are persisted in SQLite.
...
- **Inference**: Any OpenAI-compatible endpoint (default: Ollama at `localhost:11434`)

## Architecture

```
Browser (SolidJS SPA)
  │  hono/client (RPC), EventSource
  │
  ▼  HTTP/JSON + SSE
Hono (Bun :8000)
  ├── /api/health                   → LLM health probe (Ollama /api/tags)
  ├── /api/sessions/*                → Session CRUD (Drizzle)   → voxpilot.db
  │   ├── GET  /{id}/stream          → persistent SSE stream (streamSSE)
  │   │     └── agent loop           → LLM (config: VOXPILOT_LLM_BASE_URL)
...
- **Auth**: None — the app is single-user self-hosted. All routes are public.
- **Config**: Environment variables with `VOXPILOT_` prefix loaded via `dotenv` in Justfile. Key settings: `llmBaseUrl` (default `http://localhost:11434/v1`), `llmApiKey` (default `ollama`), `llmDefaultModel` (default `qwen3-coder:32b`), `dbPath` (default `voxpilot.db`), `workDir` (defaults to cwd — root for all tool file access), `maxAgentIterations` (default 25).
...
- **Agent loop** (`services/agent.ts`): Async generator that yields SSE event objects. Creates a single OpenAI client (with `llmBaseUrl` and `llmApiKey` from config), streams the completion, accumulates tool-call deltas, executes tools via the registry, persists all messages (assistant w/ tool_calls, tool results), and loops until the LLM responds with text or hits the iteration cap. Errors from tool execution are fed back to the LLM as tool-result content so it can self-correct. On completion, the `done` event includes `html` — the full accumulated text rendered to HTML via the markdown service.
- **LLM health indicator**: `GET /api/health` probes Ollama's `/api/tags` endpoint and returns `{ status, app_name, llm, models, defaultModel }`. The frontend header polls this every 30 seconds and shows a green/red dot with the model name or "LLM offline".
...
| **Self-hosted LLM via Ollama** | OpenAI-compatible endpoint at `localhost:11434`; zero-latency, private, any model. GitHub Models removed entirely. |
| **No authentication** | Single-user self-hosted app — no OAuth flow needed. Removing auth simplified the codebase by ~200 lines. |
...
| **Backend owns history** | Frontend sends only content + optional model; single source of truth. Stream replays full history on connect. |
...
| **OpenAI SDK with configurable baseURL** | Compatible API; any OpenAI-compatible endpoint works via `VOXPILOT_LLM_BASE_URL` |
